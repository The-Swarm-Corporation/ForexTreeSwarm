            ...
            for article in articles[:10]:  # Get latest 10 articles
                try:
                    news_item = {
                        ...
                        'currency': article.get('data-currency', 'General'),
...
class ForexForestSystem:
    ...
    def __init__(self):
        ...
        self.sentiment_tree = Tree(
            tree_name="Sentiment Analysis",
            agents=sentiment_agents
        )
        self.strategy_tree = Tree(
            tree_name="Strategy Coordination",
        ...
        while True:
            try:
                ...
                if "recommendations" in analysis:
                    await self.process_trading_signals(analysis["recommendations"])
    ...
    async def process_trading_signals(self, recommendations: List[Dict]):
        """Process and log trading signals from analysis"""
        try:
            for rec in recommendations:
                logger.info(f"Trading Signal: {rec['pair']} - {rec['action']}")
